...
class TestInstallUtils:
    ...
    def test_install_missing_packages(self):
        ...
        with patch('install_utils.run_command') as mock_run:
            ...
            # Should call run_command with the correct arguments
    ...
    def test_install_packages(self):
        """Test the install_packages function."""
        # Create fresh mocks for this test
        with patch('install_utils.check_package_installed') as mock_check_package, \
             patch('install_utils.install_missing_packages') as mock_install_missing, \
             patch('builtins.print'):  # Suppress print output during tests
            
            # Scenario 1: All packages are installed
            mock_check_package.return_value = True
            
            # Use a modified package list for testing
            with patch.object(constants, 'REQUIRED_PKGS', ['pkg1']), \
                 patch.object(constants, 'REQUIRED_PYTHON_PKGS', []):
                install_packages('debian')
                mock_install_missing.assert_not_called()
            
            # Reset mocks for scenario 2
            mock_check_package.reset_mock()
            mock_install_missing.reset_mock()
            
            # Scenario 2: One package is missing
            # Create a side effect function that returns False for 'missing-pkg'
            def side_effect(pkg):
                print(f"Mock check_package_installed called with: {pkg}")
                return pkg != 'missing-pkg'
                
            mock_check_package.side_effect = side_effect
            
            # Use a test package list with one missing package
            with patch.object(constants, 'REQUIRED_PKGS', ['existing-pkg', 'missing-pkg']), \
                 patch.object(constants, 'REQUIRED_PYTHON_PKGS', []):
                ...
                # Print debug info
                print(f"Mock calls: {mock_install_missing.mock_calls}")
                print(f"Check package side effect working: {not mock_check_package('missing-pkg')}")
                
                # Verify install_missing_packages was called with the correct arguments
                mock_install_missing.assert_called_once_with('debian', ['missing-pkg'])

    @patch('install_utils.run_command')
    def test_install_missing_packages_called(self, mock_run_command):
        ...
        def simplified_install_packages(distro):
            ...
            print("Simulating missing packages: pkg1")  # Debug statement
        ...
        with patch('install_utils.install_missing_packages') as mock_install_missing:
            # Call our simplified function
            simplified_install_packages('debian')

            # Verify that install_missing_packages was called
            print(f"Mock install_missing_packages calls: {mock_install_missing.mock_calls}")  # Debug statement
            mock_install_missing.assert_called_once_with('debian', ['pkg1'])

    @patch('install_utils.run_command')
    def test_install_packages_detects_missing(self, mock_run_command):
        ...
        def simplified_check_and_install(distro, pkgs, check_results):
            ...
            for i, pkg in enumerate(pkgs):
                ...
                if not check_results[i]:
                    print(f"Package {pkg} is missing")  # Debug statement
                    missing.append(pkg)
        ...
        with patch('install_utils.install_missing_packages') as mock_install_missing:
            ...
            # Verify that install_missing_packages was called with the correct arguments
            print(f"Mock install_missing_packages calls: {mock_install_missing.mock_calls}")  # Debug statement
            mock_install_missing.assert_called_once_with('debian', ['pkg2'])
